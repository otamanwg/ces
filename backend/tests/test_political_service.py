...
def _make_player(
    ...
    """Create a player with education/reputation (fixture default lacks these)."""
    player = Player(
        city_id=city.id,
...
def _make_district(db, city, name: str = "TestDistrict", desirability: float = 30.0) -> CityDistrict:
    """Create a district with a custom desirability (fixture default has crime_risk=50)."""
...
class TestOfficeHierarchy:
    def test_hire_worker(self, db, city):
    ...
    def test_hire_invalid_position(self, db, city):
        player = _make_player(db, city, "worker")
        result = hire_office_worker(db, city, player, "boss", "economy", game_day=1)
        assert result["success"] is False

    def test_hire_worker_without_department_fails(self, db, city):
        player = _make_player(db, city, "worker")
        result = hire_office_worker(db, city, player, "worker", None, game_day=1)
        assert result["success"] is False

    def test_cannot_hire_twice(self, db, city):
        player = _make_player(db, city, "worker")
    ...
    def test_office_days_accumulate(self, db, city):
        player = _make_player(db, city, "worker")
        hire_office_worker(db, city, player, "worker", "economy", game_day=1)
        db.flush()
...
class TestCandidacyRequirements:
    def test_insufficient_office_days_blocks(self, db, city):
    ...
    def test_sufficient_office_days_allows(self, db, city):
    ...
    def test_insufficient_education_blocks(self, db, city):
...
class TestElections:
    ...
    def test_vote_for_candidate(self, db, city):
    ...
    def test_cannot_vote_twice(self, db, city):
    ...
    def test_conclude_election_picks_winner(self, db, city):
    ...
    def test_conclude_before_end_fails(self, db, city):
        start_election(db, city, game_day=1)
        db.flush()
        election = db.query(MayorElection).first()
...
class TestVoteOfNoConfidence:
    def test_no_confidence_starts_new_election(self, db, city):
    ...
    def test_no_confidence_too_early_fails(self, db, city):
        mayor = _make_player(db, city, "mayor")
        city.mayor_player_id = mayor.id
        city.mayor_term_started_game_day = 1
        db.flush()
...
class TestVoteBribery:
    def test_offer_bribe(self, db, city):
    ...
    def test_respond_bribe_accept(self, db, city):
        briber = _make_player(db, city, "briber", balance=10000.0)
    ...
    def test_respond_bribe_reject(self, db, city):
        briber = _make_player(db, city, "briber", balance=10000.0)
        voter = _make_player(db, city, "voter", balance=100.0)
        start_election(db, city, game_day=1)
        db.flush()
    ...
    def test_respond_bribe_report(self, db, city):
        briber = _make_player(db, city, "briber", balance=10000.0)
        voter = _make_player(db, city, "voter", balance=100.0)
        start_election(db, city, game_day=1)
        db.flush()
...
class TestAiMayor:
    def test_ai_mayor_invests_in_worst_district(self, db, city):
...
class TestPoliticalApi:
    def test_hire_office_endpoint(self, db, city):
    ...
    def test_start_election_endpoint(self, db, city):
    ...
    def test_get_election_endpoint_no_active(self, db, city):
