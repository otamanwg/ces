...
def _make_bank(db, city: City, cash: float = 50000.0, owner: Player | None = None) -> Business:
    bank = Business(
        city_id=city.id,
        name="TestBank",
        type="bank",
...
def _make_player(db, city: City, username: str = "testplayer", balance: float = 5000.0) -> Player:
    player = Player(
        city_id=city.id,
...
def _make_bankrupt_business(db, city: City, owner: Player | None = None) -> Business:
    business = Business(
        city_id=city.id,
        name="BankruptBiz",
        type="shop",
...
class TestBankBlueprint:
    def test_bank_blueprint_exists_after_seed(self):
...
class TestDeposits:
    ...
    def test_create_deposit_insufficient_funds(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            bank = _make_bank(db, city)
    ...
    def test_create_deposit_non_bank_business(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            shop = Business(
    ...
    def test_deposit_interest_accrues(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            bank = _make_bank(db, city)
...
class TestLoans:
    def test_issue_loan_success(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
    ...
    def test_issue_loan_bank_insufficient(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
    ...
    def test_issue_loan_existing_active_blocks(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            bank = _make_bank(db, city, 50000.0)
    ...
    def test_repay_loan_partial(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            bank = _make_bank(db, city, 50000.0)
    ...
    def test_loan_interest_accrues(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            bank = _make_bank(db, city, 50000.0)
...
class TestBankruptcyAuctions:
    ...
    def test_trigger_auction_already_exists(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            business = _make_bankrupt_business(db, city)
