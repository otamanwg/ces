...
def _make_city(db) -> City:
    city = City(name="G8TestCity", treasury_balance=Decimal("100000.00"), game_day=0)
    db.add(city)
    db.flush()
    return city
...
def _make_district(db, city, name="G8District") -> CityDistrict:
...
class TestPolice:
    def test_hire_patrol(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            player = _make_player(db, city, "patrol")
    ...
    def test_cannot_hire_twice(self):
        ...
        try:
            city = _make_city(db)
            player = _make_player(db, city, "patrol2")
            from backend.app.services.police_service import hire_police_officer
    ...
    def test_promote_requires_investigations(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            player = _make_player(db, city, "patrol3")
    ...
    def test_promote_to_detective(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            player = _make_player(db, city, "patrol4")
            from backend.app.services.police_service import (
                get_officer,
                hire_police_officer,
                promote_officer,
    ...
    def test_accept_bribe_creates_corruption_log(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
...
class TestCourt:
    def test_create_case_fine(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            defendant = _make_player(db, city, "defendant")
    ...
    def test_create_case_criminal_creates_sentence(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            defendant = _make_player(db, city, "defendant2")
            corruption = CorruptionLog(
    ...
    def test_file_appeal(self):
        ...
        try:
            city = _make_city(db)
            defendant = _make_player(db, city, "defendant3")
            corruption = CorruptionLog(
                incident_type="minor_bribe",
                perpetrator_id=defendant.id,
                amount=100.0,
    ...
    def test_bribe_judge_success(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
...
class TestPrison:
    def test_serve_day(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            player = _make_player(db, city, "inmate")
...
class TestPress:
    def test_start_investigation(self):
        ...
        try:
            city = _make_city(db)
            journalist = _make_player(db, city, "journalist")
    ...
    def test_publish_article(self):
        ...
        try:
            city = _make_city(db)
            journalist = _make_player(db, city, "journalist2")
...
class TestLawyer:
    ...
    def test_lawyer_level_grows(self):
        db = make_test_session(TEST_DATABASE_URL)
        try:
            city = _make_city(db)
            lawyer = _make_player(db, city, "lawyer2")
