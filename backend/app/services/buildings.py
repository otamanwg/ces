...
from backend.app.models import Building, Business, City, Player
...
def activate_building_application(db: Session, player: Player, application_id: UUID) -> dict:
    application = BuildingApplicationRepository(db).get_by_id_for_update(application_id)
    ...
    existing_building = BuildingRepository(db).get_by_parcel_or_application(parcel.id, application.id)
...
def get_player_building_portfolio(db: Session, player: Player) -> list[Building]:
    return BuildingRepository(db).get_by_owner_with_details(player.id)


def open_building_operations(db: Session, player: Player, building_id: UUID) -> dict:
    building = BuildingRepository(db).get_by_id_for_update(building_id)
...
def process_daily_building_upkeep(db: Session, city: City) -> dict:
    ...
    buildings = BuildingRepository(db).list_active_for_upkeep(city.id)
