...
class PlayerRepository(BaseRepository[Player]):
    ...
    def list_by_city_with_hostel(self, city_id: UUID) -> list[Player]:
        return self.db.query(Player).options(joinedload(Player.hostel_rented)).filter(Player.city_id == city_id).all()

    def count_hungry_in_city(self, city_id: UUID, threshold: int = 70) -> int:
        from sqlalchemy import func

        return (
            self.db.query(func.count(Player.id)).filter(Player.city_id == city_id, Player.hunger >= threshold).scalar()
        )
